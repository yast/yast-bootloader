...
module Bootloader
  # Represents bls compatile bootloaders with efi target
  class BlsBootloader < BootloaderBase
    include Yast::Logger
    include Yast::I18n
    ...
    def merge(other)
      log.info "merging: secure_boot: #{secure_boot}=>#{other.secure_boot}"
      ...
      log.info "merging result: secure_boot: #{secure_boot}"
      log.info "                mitigations: #{cpu_mitigations.to_human_string}"
      log.info "                kernel_params: #{kernel_params.serialize}"
    end
