...
module Yast
  class BootloaderClass < Module
    ...
    def enableSELinuxPAM
      ...
      out = Convert.to_map(SCR.Execute(path(".target.bash_output"), cmd_disable_aa))
      Builtins.y2debug("result of disabling the AppArmor PAM module is %1", out)

      out = Convert.to_map(SCR.Execute(path(".target.bash_output"), cmd_enable_se))
    ...
    def disableSELinuxPAM
      ...
      out = Convert.to_map(SCR.Execute(path(".target.bash_output"), cmd_disable_se))
      Builtins.y2debug("result of disabling the SELinux PAM module is %1", out)

      out = Convert.to_map(SCR.Execute(path(".target.bash_output"), cmd_enable_aa))
